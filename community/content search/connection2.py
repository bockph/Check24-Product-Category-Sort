...
def index_without_embedding(index_name):
    ...
    for idx, post in tqdm(posts.iterrows()):
        id += 1
        if post['content'].encode('ascii','ignore').decode('utf-8','ignore').strip()=="":continue
        document = {
            "content_type": "post",
            ...
            "content": post['content'].encode('ascii','ignore').decode('utf-8','ignore'),
    ...
    for idx, comment in tqdm(comments.iterrows()):
        id += 1
        if comment['content'].encode('ascii','ignore').decode('utf-8','ignore').strip()=="":continue
        document = {
            ...
            "content": comment['content'].encode('ascii','ignore').decode('utf-8','ignore'),
