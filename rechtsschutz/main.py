...
def process_check_result(div, monthly):
    ...
    else: result_dict['Preis Jahr'] = float(price)
...
def create_verivox_url(contract_runtime,marital_status,employement_status,birthdate,zipcode,selfparticipation,module_priv,
    ...
    sections = '&'.join(["tariffSections["+str(idx)+"]="+tariff_type for idx,tariff_type in enumerate(tariff_sections)])
    ...
    url="https://service.verivox.de/applications/insurance/service/search/lpi/search?eventHash=e975d19ed398df57f64618671cc3d0d0&isAdditionalQuestionsChange=false&" \
        ...
        "wayOfPayment=1&minimumContractTerm="+str(contract_runtime)+"&" \
...
if __name__=="__main__":
    ...
    verivox_url_threeyear = create_verivox_url(3, marital_status, employement_status, birthdate, zipcode,
                                             selfparticipation, module_priv,
                                             module_job, module_traffic, module_living)
    check_url_month = create_check_url(True,marital_status,employement_status,birthdate,zipcode,selfparticipation,module_priv,
                     module_job,module_traffic,module_living,module_rental)
    check_url_year =create_check_url(False,marital_status,employement_status,birthdate,zipcode,selfparticipation,module_priv,
                     module_job,module_traffic,module_living,module_rental)
    ...
    pd_final_verivox_data.to_csv("./verivox_" + case_name + ".csv")
   ...
   #
    ...
    pd_final_check_data = pd.merge(pd_check_month_data,pd_check_year_data[['Anbieter','Tarif','Preis Jahr']],on=['Anbieter','Tarif'])
    pd_final_check_data.to_csv("./check_"+case_name+".csv")
    #final_data[['Anbieter','Tarif']].to_csv("./check_names_1.csv")
